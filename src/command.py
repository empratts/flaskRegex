...
class TrackerCommands(Cmd):
    ...
    with open(f'{FILE_PATH}/../data/combos.json', "r", encoding='utf-8') as f:
        combos:dict[str,dict[str,list[str]]] = json.load(f)
    
    with open(f'{FILE_PATH}/../data/flask_mods.json', "r", encoding='utf-8') as f:
        affix = json.load(f)

    prefix:list[str] = [p["Name"].lower() for p in affix if p["Prefix"] == True]
    suffix:list[str] = [s["Name"].lower() for s in affix if s["Prefix"] == False]

    with open(f'{FILE_PATH}/../data/flask_bases.json', "r", encoding='utf-8') as f:
        bases:list[str] = json.load(f)

    bases:set[str] = {b.lower().replace(" flask", "") for b in bases}

    signal.signal(signal.SIGALRM, sig_handler)

    def __init__(self):
        history_file = f'{TrackerCommands.FILE_PATH}/../data/command_history.dat'
        super().__init__(persistent_history_file=history_file, persistent_history_length=1000)


        self.wanted_affixes: set[str] = set()

        db_conn = sqlite3.connect(f'{TrackerCommands.FILE_PATH}/../data/item.db')
    ...
    def do_add(self, args):
        ...
        prefix_id, prefix_grp, prefix_lvl = self.getPrefixInfoFromDB(args.prefix, db_cur)
        base_id = self.getBaseIDFromDB(args.base, db_cur)
        suffix_id, suffix_grp, suffix_lvl = self.getSuffixInfoFromDB(args.suffix, db_cur)
    ...
    def do_regex(self, args):
        ...
        wanted_flasks_for_base = self.genWatnedFlasksForBase(self.wanted_flasks, args.base)

        if wanted_flasks_for_base:

            found_cliques = self.computeCliques(wanted_flasks_for_base)
            ...
            wanted_flask_strings = {f"{w[0]} {args.base} flask {w[1]}" for w in wanted_flasks_for_base}
    ...
    def do_export(self, args):
        ...
        with open(f'{TrackerCommands.FILE_PATH}/../data/export.json', "w", encoding='utf-8') as f:
            json.dump(wanted, f)
    ...
    def do_import(self, args):
        ...
        with open(f'{TrackerCommands.FILE_PATH}/../data/export.json', "r", encoding='utf-8') as f:
            items = json.load(f)
        ...
        for i in items:
            ...
            prefix_id, prefix_grp, prefix_lvl = self.getPrefixInfoFromDB(i["prefix"], db_cur)
            base_id = self.getBaseIDFromDB(i["base"], db_cur)
            suffix_id, suffix_grp, suffix_lvl = self.getSuffixInfoFromDB(i["suffix"], db_cur)
    ...
    @staticmethod
    def genWatnedFlasksForBase(wanted_flasks:set[str], base:str) -> set[tuple[str,str]]:
        return {(w[0], w[2]) for w in wanted_flasks if w[1] == base}

    @staticmethod
    def computeCliques(wanted_flasks_for_base: set[tuple[str,str]]) -> list[set[tuple[str,str]]]:
        ...
        for i1 in wanted_flasks_for_base:
            for i2 in wanted_flasks_for_base:
                if i1 != i2 and not G.has_edge(i1, i2) and (i1[0], i2[1]) in wanted_flasks_for_base and (i2[0], i1[1]) in wanted_flasks_for_base:
                    G.add_edge(i1, i2)
        
        found_cliques: list[set[tuple[str,str]]] = []
    ...
    @classmethod
    def genRegexForClique(cls, clique:set[str], base: str) -> str:
        ...
        affixes = [f"^{p}" for p in cls.prefix] + [f"{s}$" for s in cls.suffix]

        base_short_name = cls.getBaseShortName(base, affixes)
    ...
    @staticmethod
    def getBestCombo(group:set[str], base: str, base_regex:str, affix_list:list[str], valid_combos:dict[str,list[str]]) -> (str, list[str]):
        
        best_combo = None
        best_remaining_names = []
        short_names = [TrackerCommands.getShortName(f, affix_list, base) for f in group] + [base_regex]
        best_option = min([TrackerCommands.foldPrefix(short_names), TrackerCommands.foldSuffix(short_names)], key=len)

        for combo, affixes in valid_combos.items():
            remaining_names = [affix for affix in group if affix not in affixes]
            short_names = [TrackerCommands.getShortName(f, affix_list, base) for f in remaining_names] + [base_regex]
            new_option = min([f"{combo}|{TrackerCommands.foldPrefix(short_names)}", f"{combo}|{TrackerCommands.foldSuffix(short_names)}"], key=len)
    ...
    @staticmethod
    def optimizeGroup(group:set[str], base: str, base_regex:str, affix_list:list[str], valid_combos:dict[str,list[str]]) -> str:
        results:list[str] = []
        remaining_names = group

        while True:
            new_combo, new_remaining_names = TrackerCommands.getBestCombo(remaining_names, base, base_regex, affix_list, valid_combos)
            if new_combo:
        ...
        short_names = [TrackerCommands.getShortName(f, affix_list, base) for f in remaining_names]

        best_fold = min([TrackerCommands.foldPrefix(short_names), TrackerCommands.foldSuffix(short_names)], key=len)
    ...
    def generateGroup(words: Iterable[str]) -> str:
        ...
        options.append(TrackerCommands.foldPrefix(words))
        options.append(TrackerCommands.foldSuffix(words))
